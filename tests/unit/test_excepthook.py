...
def test_register_and_restore_except_handler():
    ...
    assert (
        original_python_handler is sys.__excepthook__
    ), "Original handler should be python builtin one"
    assert (
        sys.excepthook is not original_python_handler
    ), "The except handler should not be the original one anymore"
    assert (
        sys.excepthook.__wrapped__ is mock_handler_1
    ), "The current except handler should be a wrapper around our callback"

    first_handler_wrapper = sys.excepthook
    previous_handler = register_exception_handler(mock_handler_2)
    assert (
        previous_handler == first_handler_wrapper
    ...
    assert (
        sys.excepthook is first_handler_wrapper
    ...
    assert (
        sys.excepthook is original_python_handler
    ), "Current except handler should be the original one"
...
def test_register_and_restore_except_handler_with_call():
    ...
    with patch("sys.excepthook") as original_python_handler:
        ...
        original_python_handler = register_exception_handler(
            mock_handler_1, call_previous_handler=False
        )
